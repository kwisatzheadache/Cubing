defmodule Trainer do
  def begin do
    ...
    dv_vector =  [Fitness.dv_mag(cube, in_cube, 0), Fitness.dv_mag(cube, in_cube, 1), Fitness.dv_mag(cube, in_cube, 1)]
    ...
    receive do
      ...
      new_cubee = Cubing.rotate(in_cube, rotation[:axis], rotation[:index])  
      new_dv_vector =  [Fitness.dv_mag(cube, new_cube, 0), Fitness.dv_mag(cube, new_cube, 1), Fitness.dv_mag(cube, new_cube, 1)]
      fitness = Enum.sum(new_dv_vector)
      Agent.update(fitness_agent, fn map -> Map.put(map, :fitness, fitness) end)
      send metadata[:self]
      {:ok, Agent.get(fitness_agent, fn map -> map end)}
    end
  end
end
